...
import type { EventDate } from '../../data/eventData';
...
interface EventCardProps {
    title: string;
    description: string;
    /** Un componente de icono de Lucide */
...
const EventCard: React.FC<EventCardProps> = ({
  ...
  const handleCelebrationType = (
    event: React.MouseEvent,
    celebrationType: string
  ) => {
    event.stopPropagation();
    const reservationForm = document.getElementById('contact');
    if (reservationForm && window.setEventTypeFromOutside) {
        ...
        return (
          <div className="mt-4 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {dates.length === 0 ? (
                <p className="text-white/80">No hay conciertos programados.</p>
              ) : (
          ...
          <div className="mt-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {celebrationTypes.map(t => (
                <button
                  key={t.id}
                  onClick={e => handleCelebrationType(e, t.label)}
            ...
            <div className="mt-4 p-4 bg-white/10 border border-white/20 rounded-lg text-white text-sm flex items-start space-x-3">
              ...
              <span>
                La información sobre los menús y las modificaciones se hará
                mediante WhatsApp al número de teléfono indicado.
              </span>
            </div>
          </div>
        );
