...
const Menus: React.FC = () => {
  ...
  const handleMenuChange = (menuType: MenuType) => setActiveMenu(menuType);
        ...
        <div className="flex flex-wrap justify-center mb-12">
          <button
            className={`px-6 py-3 mx-2 mb-2 rounded-full text-sm font-medium transition-all ${
              activeMenu === 'daily' ? 'bg-brown-700 text-white' : 'bg-white text-neutral-700 hover:bg-neutral-100'
            }`}
          ...
          <button
            className={`px-6 py-3 mx-2 mb-2 rounded-full text-sm font-medium transition-all ${
              activeMenu === 'weekend' ? 'bg-brown-700 text-white' : 'bg-white text-neutral-700 hover:bg-neutral-100'
            }`}
            onClick={() => handleMenuChange('weekend')}
          >Menú Fin de Semana</button>
        </div>

        <div className={`space-y-12 transition-all duration-1000 transform ${inView ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
          ...
          <div>
            ...
            <div className="flex flex-col gap-8 sm:grid sm:grid-cols-2 lg:grid-cols-3">
              {extraerPrimero(menuToShow).map((item, i) => (
                <MenuCard
                  key={i}
                  title={item.titulo}
          ...
          <div>
            ...
            <div className="flex flex-col gap-8 sm:grid sm:grid-cols-2 lg:grid-cols-3">
              {extraerSegundo(menuToShow).map((item, i) => (
                <MenuCard
                  key={i}
                  title={item.titulo}
                  description={item.descripcion}
          ...
          <div>
            ...
            <div className="flex flex-col gap-8 sm:grid sm:grid-cols-2 lg:grid-cols-3">
              {extraerPostre(menuToShow).map((item, i) => (
                <MenuCard
                  key={i}
                  title={item.titulo}
                  description={item.descripcion}
